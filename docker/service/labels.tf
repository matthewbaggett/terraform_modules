locals {
  ...
  traefik_labels = merge(
    (var.traefik == null ? {
      ...
      "traefik.http.routers.${local.service_name}.service"              = "${local.service_name}@swarm"
      ...
      "traefik.http.routers.${local.service_name}_ssl.service"          = var.traefik.ssl ? "${local.service_name}_ssl@swarm" : null
