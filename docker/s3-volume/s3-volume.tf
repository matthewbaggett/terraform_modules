...
module "swarm_exec" {
  source = "../service"
  image = var.image_swarm_exec
  stack_name = var.stack_name
  service_name = "s3fs-${substr(local.volume_name,0,30)}"
  mounts  = { "/var/run/docker.sock" = "/var/run/docker.sock" }
  global = true
  command = ["sh", "-c", "docker plugin install --alias ${local.alias} ${var.image_s3_plugin} --grant-all-permissions --disable AWSACCESSKEYID=${local.access_key} AWSSECRETACCESSKEY=${local.secret_key} DEFAULT_S3FSOPTS='allow_other,uid=1000,gid=1000,nomultipart'; docker plugin enable ${local.alias}"]
}
